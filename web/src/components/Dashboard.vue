<template>
      ...
      <el-row :gutter="20" class="mb-4">
        <el-col :span="6">
          <el-card shadow="hover" class="data-card">
            <div class="card-icon green-bg"><el-icon><Monitor /></el-icon></div>
            <div class="card-info">
              <div class="card-label">节点健康度</div>
              <div class="card-value">{{ onlineNodes }} / {{ totalNodes }}</div>
              <el-progress :percentage="nodeHealthRate" :show-text="false" status="success" :stroke-width="6" class="mt-2" />
            </div>
          </el-card>
        </el-col>
        <el-col :span="6">
          <el-card shadow="hover" class="data-card">
            <div class="card-icon blue-bg"><el-icon><Platform /></el-icon></div>
            <div class="card-info">
              <div class="card-label">服务实例 (Running)</div>
              <div class="card-value">{{ runningInstances }} / {{ totalInstances }}</div>
              <el-progress :percentage="instanceHealthRate" :show-text="false" :stroke-width="6" class="mt-2" />
            </div>
          </el-card>
        </el-col>
        <el-col :span="6">
          <el-card shadow="hover" class="data-card">
            <div class="card-icon orange-bg"><el-icon><Cpu /></el-icon></div>
            <div class="card-info">
              <div class="card-label">集群平均负载</div>
              <div class="card-value">{{ avgCpu }}% <span class="sub-val">CPU</span></div>
              <div class="sub-text">MEM: {{ avgMem }}%</div>
            </div>
          </el-card>
        </el-col>
        <el-col :span="6">
          <el-card shadow="hover" class="data-card" :class="{ 'alert-mode': activeAlerts > 0 }">
            <div class="card-icon red-bg">
               <el-badge :value="activeAlerts" :max="99" :hidden="activeAlerts===0" class="badge-offset">
                 <el-icon><BellFilled /></el-icon>
               </el-badge>
            </div>
            <div class="card-info">
              <div class="card-label">活跃告警</div>
              <div class="card-value" :class="{'text-danger': activeAlerts>0}">{{ activeAlerts }}</div>
              <div class="sub-text">{{ activeAlerts > 0 ? '请立即处理' : '系统运行正常' }}</div>
            </div>
          </el-card>
        </el-col>
      </el-row>
      ...
      <el-row :gutter="20" class="row-lists">
        ...
        <el-col :span="12">
          <el-card shadow="never" class="list-card">
            ...
            <el-table :data="alertList" style="width: 100%" size="small" :show-header="false">
        ...
        <el-col :span="12">
          <el-card shadow="never" class="list-card">
            ...
            <el-table :data="logList" style="width: 100%" size="small" :show-header="false">
               <el-table-column width="140">
                 <template #default="scope">
  ...
  const onlineNodes = computed(() => wsStore.nodes.filter(n => n.status === 'online').length)
  const nodeHealthRate = computed(() => totalNodes.value ? (onlineNodes.value / totalNodes.value) * 100 : 0)
  ...
  const runningInstances = computed(() => allInstances.value.filter(i => i.status === 'running').length)
  const instanceHealthRate = computed(() => totalInstances.value ? (runningInstances.value / totalInstances.value) * 100 : 0)
  
  const avgCpu = computed(() => {
    if (onlineNodes.value === 0) return 0
    const sum = wsStore.nodes.reduce((acc, cur) => acc + (cur.status==='online' ? cur.cpu_usage : 0), 0)
    return (sum / onlineNodes.value).toFixed(1)
  })
  const avgMem = computed(() => {
    if (onlineNodes.value === 0) return 0
    const sum = wsStore.nodes.reduce((acc, cur) => acc + (cur.status==='online' ? cur.mem_usage : 0), 0)
    return (sum / onlineNodes.value).toFixed(1)
  })
  ...
  const updateChart = () => {
    ...
    lineOption.value = {
      ...
      xAxis: { type: 'category', boundaryGap: false, show: false }, // 隐藏X轴文字保持简洁
  ...
  <style scoped>
  .dashboard { padding: 20px; overflow-y: auto; background-color: var(--el-bg-color-page); }
  
  /* 卡片通用 */
  .data-card {
    display: flex; align-items: center; border: none; cursor: pointer; transition: transform 0.2s;
  }
  .data-card:hover { transform: translateY(-3px); }
  .data-card :deep(.el-card__body) { display: flex; align-items: center; padding: 20px; width: 100%; }
  
  /* 图标色块 */
  .card-icon {
    width: 56px; height: 56px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px; color: #fff; margin-right: 16px;
  }
  ...
  .card-info { flex: 1; }
  .card-label { font-size: 14px; color: var(--el-text-color-secondary); margin-bottom: 4px; }
  .card-value { font-size: 24px; font-weight: bold; color: var(--el-text-color-primary); }
  .sub-val { font-size: 12px; color: #999; margin-left: 4px; }
  .sub-text { font-size: 12px; color: var(--el-text-color-secondary); margin-top: 4px; }
  
  .mt-2 { margin-top: 8px; }
  .mb-4 { margin-bottom: 20px; }
  .mx-2 { margin: 0 8px; }
  
  .badge-offset :deep(.el-badge__content) { transform: translate(10px, -10px); }
  ...
  .time-text { font-family: monospace; color: #999; }
  .font-bold { font-weight: bold; }
  .text-danger { color: var(--el-color-danger); }
  .text-success { color: var(--el-color-success); }
  .text-gray { color: #ccc; }
  .log-action { font-weight: 500; color: var(--el-color-primary); }
  </style>